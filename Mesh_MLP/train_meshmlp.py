...
# path_save_model = "..../MeshMlp_bu_G0.pth" 
...
def get_metrics(model, test_loader, device):
    ...
    with torch.no_grad():
        for batch_x, batch_y in test_loader:
            ...
            outputs = model(batch_x)
            preds   = (outputs.squeeze() > 0.5).float()  # Use 0.5 as threshold for binary classification
            all_preds.extend(preds.cpu().numpy())
            all_labels.extend(batch_y.cpu().numpy())
...
model = Net(C_in=feature_dim, C_out=num_classes, drop_path_rate=drop_prob, k_eig_list=k_eig_list).to(device)
